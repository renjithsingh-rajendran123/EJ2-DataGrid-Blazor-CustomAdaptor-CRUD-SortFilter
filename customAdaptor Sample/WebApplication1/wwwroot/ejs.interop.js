...
window.ejsInterop = {
    ...
    updateModel: async function (key, newValue, oldValue) {
        try {
            ...
            if (this.isRendered && !this.preventUpdate && ejsInterop.compareValues(newValue, oldValue)) {
                var newObj = {};
    ...
    parseRevive: function (key, value, templateRefs) {
        ...
        if (key == "" && value.dataSource_custom) {
    ...
    modelJsonParseRevive: function (key, value) {
        ...
        } else if (typeof value === "string" && key === "query") {
            return eval(value);
        }
        return value;
    },
        ...
        window.BlazorAdaptor = class BlazorAdaptor extends ejs.data.UrlAdaptor {
            ...
            doAjaxRequest(args) {
                ...
                if (args.requestType === "insert") {
                    dm.dotnetInstance.invokeMethodAsync('Insert', JSON.stringify(args.data), dm.key, args.position).then(data => {
                        defer.resolve(data);
                    });
                }
